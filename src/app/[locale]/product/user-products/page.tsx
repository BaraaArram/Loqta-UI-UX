      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 relative">
        <section className="bg-cardC rounded-3xl shadow-2xl p-8 border border-muted/30">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-muted text-lg font-semibold">{products.length} {products.length === 1 ? t('product') : t('products')}</span>
          </div>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12">
              <img 
                ...
                onError={(e) => {
                  ...
                  target.src = "https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=128&h=128&fit=crop&crop=center";
                }}
              />
              <NoData message={t('no_products_message')} action={<Link href="/product" className="text-accentC hover:underline font-bold">{t('add_first_product')}</Link>} />
            </div>
          ) : (
              ...
              {products.map((item) => (
                <div key={item.id} className="flex items-center gap-4 bg-cardC/80 rounded-lg p-3 shadow-sm border border-muted/20 hover:shadow-lg transition group">
                  <img 
                    ...
                    alt={item.name} 
                    className="h-14 w-14 object-cover rounded-lg border border-muted group-hover:scale-105 transition"
                    onError={(e) => {
                      // Fallback to online image if local image fails
                      const target = e.target as HTMLImageElement;
                      target.src = "https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=56&h=56&fit=crop&crop=center";
                    }}
                  />
                  ...
                  <Link href={`/product/${item.slug}`} className="px-4 py-2 bg-accentC text-cardC rounded-lg font-bold hover:bg-accentC/90 transition text-sm focus:outline-none focus:ring-2 focus:ring-accentC">{t('view')}</Link>
                  <button
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl w-full max-w-3xl flex flex-col md:flex-row overflow-hidden border border-muted/30 relative">
              ...
              <div className="w-full md:w-1/2 flex flex-col items-center justify-center bg-gradient-to-br from-accentC/10 via-cardC/40 to-bodyC p-4 sm:p-8 relative">
                <div className="w-48 h-48 sm:w-64 sm:h-64 md:w-80 md:h-80 flex items-center justify-center bg-white/80 rounded-2xl shadow-xl border-2 border-white dark:border-cardC">
                  {editThumbnailFile ? (
                    ...
                    <span className="text-muted text-lg">{t('image_preview')}</span>
                  )}
                </div>
                <label className="mt-4 w-full">
                  <span className="block font-medium text-heading mb-2">{t('product_image')}</span>
                  <input
                    type="file"
                    accept="image/*"
              ...
              <div className="w-full md:w-1/2 flex flex-col gap-6 p-4 sm:p-8 items-center md:items-start justify-center relative">
                ...
                <form onSubmit={handleEditSubmit} className="w-full flex flex-col gap-4">
                  <input name="name" value={editForm.name} onChange={handleEditChange} placeholder={t('product_name')} className="w-full p-3 border border-muted rounded-lg bg-bodyC text-heading focus:ring-2 focus:ring-accentC text-lg font-semibold" required />
                  <textarea name="description" value={editForm.description} onChange={handleEditChange} placeholder={t('description')} className="w-full p-3 border border-muted rounded-lg bg-bodyC text-heading focus:ring-2 focus:ring-accentC text-base" rows={4} required />
                  <div className="flex flex-col sm:flex-row gap-4">
                    <input name="price" value={editForm.price} onChange={handleEditChange} placeholder={t('price')} type="number" step="0.01" className="w-full sm:w-1/2 p-3 border border-muted rounded-lg bg-bodyC text-heading focus:ring-2 focus:ring-accentC text-lg font-bold" required />
                    <input name="stock" value={editForm.stock} onChange={handleEditChange} placeholder={t('stock')} type="number" className="w-full sm:w-1/2 p-3 border border-muted rounded-lg bg-bodyC text-heading focus:ring-2 focus:ring-accentC text-lg font-bold" required />
                  </div>
                  <div className="w-full">
                    <label className="block mb-1 font-medium text-heading">{t('category')}</label>
                    <select
                      name="category"
                  ...
                  <div className="w-full">
                    <label className="block mb-1 font-medium text-heading">{t('tags')}</label>
                    <div className="flex flex-col sm:flex-row gap-2 mb-2">
                      <input type="text" value={editForm.tagInput || ''} onChange={handleEditTagInput} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); handleEditAddTag(); } }} placeholder={t('add_tag_placeholder')} className="flex-1 p-3 border border-muted rounded-lg bg-bodyC text-heading focus:ring-2 focus:ring-accentC" />
                      <button type="button" onClick={handleEditAddTag} className="w-full sm:w-auto px-3 py-2 bg-accentC text-cardC rounded-lg font-bold hover:bg-accentC/90 transition mt-2 sm:mt-0">{t('add')}</button>
                    </div>
                  ...
                  <button type="submit" className="w-full bg-accentC text-cardC py-3 rounded-lg font-bold text-lg hover:bg-accentC/90 transition mt-2" disabled={editLoading}>{editLoading ? t('saving') : t('save_changes')}</button>
